...
# Q8
# df=pd.read_csv("F:\AI_aaaaa\AI_Assignments\pandas\AS1_dataset_mtcars.csv")
# print(df['disp'])
# print(df.columns)
# print(df[['disp']])
# print(df.iloc[:,'disp'])
...
# Q9
titanic=pd.read_csv("F:\AI_aaaaa\AI_Assignments\pandas\AS1titanic.csv")
# titanic=titanic.T.drop_duplicates(keep="first")
# titanic=titanic.drop_duplicates(keep ="first")
titanic=titanic.drop_duplicates(keep ="last")
...
def filtered_customers(df): 
    ...
    # YOUR CODE GOES HERE 
    # Filter the dataframe having ratings>=6 and choose the required columns return new_df
    new_df= df.loc[df['rating']>=6]
    return new_df[['profession', 'gender', 'age', ]]

a=filtered_customers(df)
# print(a)
